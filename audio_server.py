    ...
    with open(LOG_FILE, "a", newline='') as f:
        ...
        if f.tell() == 0:
            writer.writerow([
                ...
                "filename", "segment_start", "segment_end", "energy_ratio", "rms"
            ])
        ...
        rms_val = float(detection_metadata.get("rms", "")) if detection_metadata and detection_metadata.get("rms", "") != "" else ""
        writer.writerow([
            ts, iso_timestamp, session_id, indicator, wav_filename, segment_start, segment_end, energy_ratio, rms_val
        ])
    return wav_filename
...
async def analyze_audio(
    ...
    if (
        ...
        ts_data, band_ratio, seg_rms = long_segments[0]
        log_name = log_event(
